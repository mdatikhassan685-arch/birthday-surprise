        ...
        const checkLandscape = () => {
            ...
            if (isLandscape) {
                if (!isSurpriseStarted) {
                    ...
                    // 🛠️ রোটেশনের পর ক্যানভাস সাইজ রিফ্রেশ
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (startBtn) {
        startBtn.addEventListener('click', async () => {
            ...
            // 🛠️ ফিক্স: ফুলস্ক্রিন হওয়ার পর উইন্ডো সাইজ পেতে ব্রাউজারকে ৩০০ms সময় দেওয়া হলো
            setTimeout(() => {
                forceResizeMatrix();
                if(typeof S !== 'undefined' && S.Drawing) S.Drawing.adjustCanvas();
                startWebsite();
...
function initMatrixRain() {
    ...
    // উইন্ডো সাইজ অনুযায়ী ক্যানভাস সেট
    matrixCanvas.width = window.innerWidth;
    matrixCanvas.height = window.innerHeight;
...
S.Drawing = (function () {
    ...
    return {
        ...
        adjustCanvas: function () {
            if(canvas) { 
                // 🛠️ ফিক্স: ফুলস্ক্রিন হওয়ার পর সঠিক উইন্ডো সাইজ নিয়ে ক্যানভাস অ্যাডজাস্ট করা
...
S.ShapeBuilder = (function () {
    ...
    function processCanvas() {
        ...
        var pixels = shapeContext.getImageData(0, 0, shapeCanvas.width, shapeCanvas.height).data,
            dots = [], x = 0, y = 0, fx = shapeCanvas.width, fy = shapeCanvas.height, w = 0, h = 0;
    ...
    return {
        circle: function (d) {
            ...
            shapeContext.beginPath(); shapeContext.arc(r * gap, r * gap, r * gap, 0, 2 * Math.PI, false);
            shapeContext.fill(); shapeContext.closePath();
        ...
        letter: function (l) {
            ...
            s = Math.min(baseFontSize, (shapeCanvas.width / shapeContext.measureText(l).width) * 0.8 * baseFontSize, (shapeCanvas.height / baseFontSize) * (!isNaN(parseFloat(l)) && isFinite(l) ? 0.8 : 0.35) * baseFontSize); 
            setFontSize(s);
            shapeContext.clearRect(0, 0, shapeCanvas.width, shapeCanvas.height);
...
S.Shape = (function () {
    ...
    function compensate() { 
        ...
        // 🛠️ ফিক্স: নাম এবং এনিমেশনগুলোকে ক্যানভাসের ঠিক মাঝখানে রাখার জন্য এই হিসাবটি খুবই গুরুত্বপূর্ণ
        cx = a.w / 2 - width / 2; 
        cy = a.h / 2 - height / 2; 
    }
...
function checkBookFinished() {
    ...
    if (currentPage === lastPageIndex && lastPage && lastPage.classList.contains('flipped')) {
        if (!isBookFinished) {
            ...
            setTimeout(() => {
                const currentSettings = window.settings || {};
                if (currentSettings.enableHeart) startHeartEffect();
            }, 1000);
        }
    }
}
...
async function showPageContent() {
    ...
    } else if(contentDisplay) { contentDisplay.classList.remove('show'); }
}

// =====================================
// 👆 7. Touch & Swipe Events
// =====================================
...
function handleTouchMove(e) {
    ...
    const deltaX = e.touches[0].clientX - startX, deltaY = e.touches[0].clientY - startY;
...
function handleSwipeMove(deltaX) {
    ...
    if (currentPageElement && !currentPageElement.classList.contains('flipped')) {
        if (deltaX < -swipeThreshold) { currentPageElement.style.transform = `rotateY(${rotation}deg)`; currentPageElement.style.boxShadow = `${rotation / 10}px 10px 20px rgba(0,0,0,${0.3 + Math.abs(rotation / 100)})`; }
    } else if (currentPage > 0) {
        const prevPageElement = document.querySelector(`.page[data-page="${currentPage - 1}"]`);
        if (prevPageElement && prevPageElement.classList.contains('flipped') && deltaX > swipeThreshold) { prevPageElement.style.transform = `rotateY(${-180 + Math.abs(rotation)}deg)`; prevPageElement.style.boxShadow = `${-rotation / 10}px 10px 20px rgba(0,0,0,${0.3 + Math.abs(rotation / 100)})`; }
    }
}
function handleTouchEnd(e) {
    if (!isDragging || isFlipping) return;
    const deltaX = e.changedTouches[0].clientX - startX, deltaY = e.changedTouches[0].clientY - startY, deltaTime = Date.now() - startTime; handleSwipeEnd(deltaX, deltaY, deltaTime);
}
function handleMouseEnd(e) {
    if (!isDragging || isFlipping) return;
    const deltaX = e.clientX - startX, deltaY = e.clientY - startY, deltaTime = Date.now() - startTime; handleSwipeEnd(deltaX, deltaY, deltaTime);
}
...
setTimeout(() => {
    ...
    if(birthdayAudio) { birthdayAudio.addEventListener('error', () => { if(musicControl) musicControl.style.display = 'none'; }); }

    document.addEventListener('visibilitychange', () => {
        if (document.hidden && isPlaying && birthdayAudio) birthdayAudio.pause();
    });
}, 1000);


// =====================================
// 🧹 9. Cleanup & Window Resize Events
// =====================================
