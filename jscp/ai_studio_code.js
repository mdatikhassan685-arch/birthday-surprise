...
function checkOrientation() {
    ...
    if (!isMobile) {
        ...
        if (isLandscape) {
            ...
            startWebsite();
            requestAutoFullscreen(); // 🎯 রোটেট করার সাথেই টাচ লিসেনার একটিভ হবে
    ...
    if (book && bookContainer) {
        ...
        requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() => {
                ...
                setTimeout(() => { if (typeof isPlaying !== 'undefined' && !isPlaying) toggleMusic(); }, 800);
            });
        });
    }
}
...
function toggleMusic() {
    ...
    if (isPlaying) {
        birthdayAudio.pause(); musicControl.innerHTML = '▶'; musicControl.classList.remove('playing'); musicControl.title = 'Play Music'; isPlaying = false;
    } else {
        birthdayAudio.play().then(() => {
            musicControl.innerHTML = '⏸'; musicControl.classList.add('playing'); musicControl.title = 'Pause Music'; isPlaying = true;
        }).catch(error => { });
    }
}
