...
function checkOrientation() {
    ...
    function updateLayout(landscape) {
    ...
    if (!isMobile) {
        ...
        const checkWindow = () => window.innerWidth > window.innerHeight;
        updateLayout(checkWindow());

        window.addEventListener('resize', () => updateLayout(checkWindow()));
        window.addEventListener('orientationchange', () => setTimeout(() => updateLayout(checkWindow()), 200));
    }
}
...
function initMatrixRain() {
    ...
    const intervalTime = isMobile ? 40 : 50; 
...
function spawnHeartPhotosCentered() {
    ...
    // ছবি না থাকলে ইনার মেমোরি থেকে ছবি নেওয়া
...
if(bookElem) {
    bookElem.addEventListener('touchstart', e => { startX = e.touches[0].clientX; startY = e.touches[0].clientY; isDragging = true; }, { passive: true });
