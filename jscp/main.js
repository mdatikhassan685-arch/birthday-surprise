...
window.playNextSequence = function() {
    ...
    if (nextEffect === 'memory') {
        ...
        if(mcScreen) mcScreen.style.display = 'flex';
        else window.playNextSequence(); 
    } 
        ...
        startHeartEffect();
        setTimeout(() => window.playNextSequence(), 4000); 
    } else {
        window.playNextSequence();
    }
}

// Memory Card Close Button Event
document.addEventListener('DOMContentLoaded', () => {
    const mcBtn = document.getElementById('mcDisplayBtn');
    if(mcBtn) {
        mcBtn.addEventListener('click', () => {
            document.getElementById('memory-card-screen').style.display = 'none';
            window.playNextSequence(); 
        });
    }
});
...
S = {
    ...
    init: function () {
        ...
        // 🎯 সিকোয়েন্স এবং মেমোরি কার্ড ডেটা লোড
        window.effectQueue = currentSettings.effectSequence ? [...currentSettings.effectSequence] : ['memory', 'book', 'hearts'];
        ...
        if(currentSettings.memoryCard) {
            ...
            if(mcTitle) mcTitle.textContent = currentSettings.memoryCard.title;
            if(mcMsg) mcMsg.textContent = currentSettings.memoryCard.message;
            if(mcBtn) mcBtn.textContent = currentSettings.memoryCard.btnText;
            if(mcImg) mcImg.src = currentSettings.memoryCard.image || './image/Birthday!/cover.jpg';
        }
                ...
                case 'gift':
                    ...
                    if (giftImage && giftImage.src && giftImage.src !== window.location.href && giftImage.src !== '') {
                        ...
                        setTimeout(() => { 
                            ...
                            window.playNextSequence(); // 🎯 Sequence Engine কল করা হলো
                        }, 3000);
                    } else {
                        window.playNextSequence(); // 🎯 Sequence Engine কল করা হলো
                    }
            ...
            setTimeout(() => {
                ...
                if (bookContainer) {
                    ...
                    setTimeout(() => {
                        ...
                        window.playNextSequence(); // 🎯 বইয়ের পড়া শেষ হলে পরের ইফেক্ট কল হবে
                    }, 800);
                }
            }, 1000);
        }
...
// 🎯 বাটনে ক্লিক করার ইভেন্ট লিসেনার
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(startBtn) {
        startBtn.addEventListener('click', () => {
            ...
            if (birthdayAudio) {
                ...
                }).catch(e => console.log("Audio auto-play prevented. User interaction needed.", e));
            }
