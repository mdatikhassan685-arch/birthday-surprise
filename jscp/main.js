...
let isUserStarted = false; // ইউজার ফুলস্ক্রিন বাটনে ক্লিক করেছে কি না তা ট্র্যাক করার জন্য
...
function checkOrientation() {
    ...
    if (!isMobile) {
        ...
        // 🚀 ফুলস্ক্রিন বাটনে ক্লিক করার ইভেন্ট
        if (startSurpriseBtn) {
            startSurpriseBtn.addEventListener('click', function() {
                // ফুলস্ক্রিন রিকোয়েস্ট (সাদা বার সরানোর জন্য)
                ...
                // 🎵 বাটন ক্লিকের সাথে সাথে মিউজিক প্লে শুরু হবে
                const birthdayAudio = document.getElementById('birthdayAudio');
                if (birthdayAudio) {
                    birthdayAudio.play().catch(e => console.log("Audio play error:", e));
                    isPlaying = true;
...
// এনিমেশন শুরু এবং বন্ধ করার ফাংশন
...
S.Color.prototype = {
    render: function () { return 'rgba(' + this.r + ',' + this.g + ',' + this.b + ',' + this.a + ')'; }
};
...
// ==========================================
// EFFECTS (Hearts, Confetti, Firework, Stars)
// ==========================================
const heartPool = [];
...
function getHeartFromPool() { return heartPool.length > 0 ? heartPool.pop() : createFloatingHeart(); }
...
function spawnHeartPhotosCentered() {
    ...
    if (typeof photoUrls !== 'undefined') photoUrls.forEach(url => preloadPhoto(url));
...
function handleTouchStart(e) {
    ...
    isDragging = true; currentTransform = 0;
}
function handleMouseStart(e) {
    if (isFlipping) return;
    startX = e.clientX; startY = e.clientY; startTime = Date.now();
    isDragging = true; currentTransform = 0; e.preventDefault();
}
...
function handleTouchEnd(e) {
    if (!isDragging || isFlipping) return;
    handleSwipeEnd(e.changedTouches[0].clientX - startX, e.changedTouches[0].clientY - startY, Date.now() - startTime);
}
function handleMouseEnd(e) {
    if (!isDragging || isFlipping) return;
    handleSwipeEnd(e.clientX - startX, e.clientY - startY, Date.now() - startTime);
}
function handleSwipeEnd(deltaX, deltaY, deltaTime) {
    isDragging = false;
    document.querySelectorAll('.page').forEach(page => { page.style.transform = ''; page.style.boxShadow = ''; });
    const velocity = Math.abs(deltaX) / deltaTime;
    if ((Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 50) || (velocity > 0.5 && Math.abs(deltaX) > 30)) {
...
function calculatePageZIndexes() {
    ...
    pages.forEach((page, physicalIndex) => {
        page.style.setProperty('--page-z-index', (totalPages - physicalIndex).toString());
