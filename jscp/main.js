...
let isSurpriseStarted = false; // এনিমেশন শুরু হয়েছে কি না ট্র্যাক করার জন্য
    ...
    if (startBtn) {
        startBtn.addEventListener('click', () => {
            ...
            if (elem.requestFullscreen) {
                elem.requestFullscreen().catch(err => console.log("Fullscreen not supported or denied."));
...
function checkOrientation() {
    ...
    initStartButton(); // বাটনের ইভেন্ট লিসেনার সেট করা

    if (!isMobile) {
        // ডেস্কটপের জন্য (সরাসরি স্টার্ট বাটন দেখাবে)
        isLandscape = true;
        if(orientationLock) orientationLock.style.display = 'flex';
        if(orientationText) orientationText.style.display = 'none'; // পিসিতে রোটেশন টেক্সট দরকার নেই
        ...
        function handleOrientationChange(matches) {
            if (matches) {
                ...
                if (!isSurpriseStarted) {
                    if(orientationLock) orientationLock.style.display = 'flex';
                    if(orientationText) orientationText.style.display = 'none';
                    if(startBtn) startBtn.style.display = 'block';
                } else {
                ...
                // পোর্ট্রেট হলে রোটেশন টেক্সট দেখাবে
                if(orientationLock) orientationLock.style.display = 'flex';
                if(orientationText) orientationText.style.display = 'block';
        ...
        handleOrientationChange(isLandscape); // Initial check
...
S.UI = (function () {
                    ...
                    if (currentSettings.enableBook === true) {
                        if (canvas && giftImage && matrixCanvas) {
                            ...
                            if (giftImage.src && giftImage.src !== window.location.href && giftImage.src !== '') {
                                ...
                                setTimeout(() => { giftImage.style.display = 'none'; showBook(); }, 3000);
                            } else {
                                showBook();
                            }
                        } else {
                        ...
                        if (canvas && matrixCanvas) { canvas.style.display = 'none'; matrixCanvas.style.display = 'none'; }
                        if (giftImage && giftImage.src && giftImage.src !== window.location.href && giftImage.src !== '') {
                            giftImage.style.display = 'block';
                            giftImage.style.animation = 'giftCelebration 2s ease-in-out';
                            if (currentSettings.enableHeart === true) setTimeout(() => startHeartEffect(), 2000);
                        } else {
                            if (currentSettings.enableHeart === true) startHeartEffect();
                        }
                    }
                    break;
                default:
    ...
    return {
        simulate: function (action) {
            if (isLandscape || !isMobile) performAction(action);
        },
