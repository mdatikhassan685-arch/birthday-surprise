...
function playMatrixAnimation() {
    ...
    // 🎯 কমান্ডটি '#gift' থেকে পরিবর্তন করে '#next' করা হলো, যাতে এনিমেশন শেষে পরের ইফেক্ট শুরু হয়
...
S = {
    initialized: false,
    init: function () {
        // 🎯 মেমোরি কার্ডের ডেটা লোড এবং ইমেজ ফিক্স
        const currentSettings = window.settings || {};
        ...
        if (currentSettings.memoryCard) {
            ...
            if (mcTitle) {
                // টাইপিং এনিমেশনের জন্য টেক্সট সেভ রাখা হলো
                window.mcTitleTextToType = currentSettings.memoryCard.title || 'Hyy Baby ❤️';
            }
            ...
            if (mcImg) {
                // 🎯 ফিক্স: মেমোরি কার্ডের ছবি না থাকলে ফাটা আইকন হাইড করবে
                if (currentSettings.memoryCard.image && currentSettings.memoryCard.image.trim() !== '') {
                    mcImg.src = currentSettings.memoryCard.image;
...
function spawnHeartPhotosCentered() {
    ...
    // 🎯 ফিক্স: কভার পেজগুলোকে ফিল্টার করে বাদ দেওয়া হলো
    const validPhotoUrls = photoUrls.filter(url => !url.includes('cover.jpg') && !url.includes('theend.jpg'));
    
    if(validPhotoUrls.length === 0) return; // যদি শুধু কভার থাকে, তবে হার্ট আসবে না
    ...
    function spawnNext() {
        if (currentIndex < maxHeartPhotos) {
            // 🎯 ফিক্স: শুধুমাত্র validPhotoUrls ব্যবহার করবে
